# Version with no weights for the skips, just add to the residual
...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    def __init__(self, config: TinyRecursiveReasoningModel_ACTV1Config) -> None:
        ...
        if self.config.output_layers > 0:
            self.output_blocks = torch.nn.ModuleList(
                [
                    TinyRecursiveReasoningModel_ACTV1Block(self.config)
                    for _i in range(self.config.output_layers)
                ]
            )
    ...
    def empty_carry(self, batch_size: int):
        # Initialize with L_init to avoid random values
        init_buffer = (
            self.L_init.view(1, 1, -1, 1)
            .expand(
                batch_size,
                self.config.seq_len + self.puzzle_emb_len,
                self.config.hidden_size,
    ...
    def reset_carry(
        ...
        return TinyRecursiveReasoningModel_ACTV1InnerCarry(
            ...
            zs=torch.where(
                reset_flag.view(-1, 1, 1, 1), self.L_init.view(1, 1, -1, 1), carry.zs
            )
        )
    ...
    def forward(
            ...
            for _H_step in range(self.config.H_cycles - 1):
                for t in range(self.max_skip):
                    ...
                    for skip in self.skips:
                        ...
                        skip_sum = skip_sum + skip_h  # sum directly, without weight
        ...
        # Final H cycle
        final_zs = [
            current_zs[..., i].detach().clone() for i in range(self.max_skip)
        ...
        for t in range(self.max_skip):
            ...
            for skip in self.skips:
                if self.config.sliding_skips:
                    skip_z = final_zs[
                        t - skip
                    ]  # Reads freshly computed values from this iteration
                else:
                    skip_z = final_zs[(t // skip) * skip]
                skip_sum = skip_sum + skip_z
            # Then process through L_level
            z = self.L_level(skip_sum, input_embeddings, **seq_info)
            final_zs[t] = z
